...
public function statuses()
{
    ...
    (new Pool($client, $dgRequests, [
        ...
        'fulfilled' => function ($response, $siteId) use (&$dgResults) {
            $dgResults[$siteId] = strtoupper(json_decode($response->getBody(), true) ?? 'OFFLINE');
        },
    ]))->promise()->wait();
    ...
    (new Pool($client, $ctrlRequests, [
        ...
        'fulfilled' => function ($response, $siteId) use (&$ctrlResults) {
            $ctrlResults[$siteId] = strtoupper(json_decode($response->getBody(), true) ?? 'OFFLINE');
        },
    ]))->promise()->wait();
    ...
    foreach ($sites as $site) {
        ...
        $statuses[$site->id] = [
            ...
            'updated_time'      => $updatedTime,   // ⭐ NEW FIELD
        ];
    }
